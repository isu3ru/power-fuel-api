...
function initLocalStorage() {
    if (localStorage.getItem('route_local_authority_id')) {
        $('#route_local_authority_id').val(localStorage.getItem('route_local_authority_id'));
    } else {
        $('#route_local_authority_id').val(route.local_authority_id);
    }
...
function sendRouteUpdateRequest() {
    let data = {
    ...
    let url = BASE_URL + '/routes/' + route.id;
    $.put(url, data, function (response) {
        if (response.data) {
            Swal.fire('Success!', 'Route updated successfully.', 'success').then((result) => {
                window.location = routesIndexRoute;
            });
            setTimeout(() => {

            }, 3000);
        } else {
            Swal.fire('Error!', 'Failed to update route details.', 'error');
...
$('#send_update_route').click(sendRouteUpdateRequest);
            ...
            routes.forEach((route) => {
                routeListTable.append(`
                    <tr>
                        ...
                        <td>
                            <button class="btn btn-sm btn-warning" data-trigger="route_edit" data-route_id="${route.id}">Edit</button>
...
function loadLocalStorage() {
    ...
    let loaded_waypoints = routeRoads.map(function (road) {
        return {
            name: road.name,
    ...
    if (localStorage.getItem('route_name') == null) {
        localStorage.setItem('route_name', route.name);
    }
    if (localStorage.getItem('route_local_authority_id') == null) {
        localStorage.setItem('route_local_authority_id', route.local_authority_id);
    }
    localStorage.setItem('start_location', JSON.stringify(loaded_start));
    localStorage.setItem('end_location', JSON.stringify(loaded_end));
    localStorage.setItem('waypoints', JSON.stringify(loaded_waypoints));
    initLocalStorage();
}
